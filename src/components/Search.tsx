...
const Search = (props: {
  ...
  useEffect(() => {
    if (ref.current && props.displaySearch) ref.current?.focus();
  }, []);

  return (
    <div className="max-h-96 overflow-y-auto overflow-x-hidden relative top-5 shadow bg-white/95 rounded-2xl dark:bg-gray-700">
            ...
            searchResult && searchResult.length > 0 ? (
              searchResult.map(i => (
                <ListItem
                  ...
                  key={i.refIndex}
                  searchResult={i.item}
                  setDisplaySearch={() => props.setDisplaySearch(false)}
                />
              ))
            ) : (
