...
import { insertSpaceBetweenChineseAndEnglish } from "../utils";
...
export const Detail = (props: {
  ...
  const getProjectorsInfo = (projectorsArray: string[]) => {
    const getProjectorFeature = (p: string) => {
      const targetProjector = projectorsData.find(
        projector => projector.name == p
      );
      if (!targetProjector) return "";

      const features = targetProjector.features
        .filter(f => f.length > 0)
        .join(", ");
      const targetAspectRatio = targetProjector.aspectRatio;
      const aspectRatio =
        targetAspectRatio != null ? `${targetProjector.aspectRatio}:1` : "";
      const res =
        " (" +
        [aspectRatio, features].filter(i => i.length > 0).join(", ") +
        ")";
  ...
  const getValue = (key: string, value: string | string[] | null) => {
    if (typeof value === "string") {
      ...
      return key === "type"
        ? getBrandDisplayName[
            value as unknown as keyof typeof getBrandDisplayName
          ]
        : value;
    }
  };
  ...
  const mapSVG = (
    <svg
      className="mr-1 w-3 h-3 text-gray-800 dark:text-gray-500"
      aria-hidden="true"
  ...
  const mapsButtonStyle =
    "text-sm tracking-tighter	flex justify-center items-center cursor-pointer bg-gray-300/40 dark:bg-gray-800/30 dark:text-gray-200 rounded-md py-1 pr-3 pl-2";
  return (
    <div className="absolute bottom-0 max-h-96 h-auto w-full justify-center flex">
      <div
        className={clsx(
        ...
        {props.current && (
          <div className="p-6">
            <h1 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">
              {insertSpaceBetweenChineseAndEnglish(props.current.properties["theatre"])}
            </h1>
                    ...
                    if (key !== "theatre" && value && value.length > 0) {
                      ...
                      if (formattedValue.length > 0) {
                        return (
                          <tr key={key}>
                            <td
                              className="pr-1 break-keep align-top text-gray-500 dark:text-gray-400 text-right py-1"
            ...
            {props.current && (
              <div className="text-center mt-5 mb-1 gap-4 sm:gap-5 md:gap-6 flex justify-center">
                <a
                  href={`https://uri.amap.com/marker?position=${props.current.coordinates.join(
                    ","
                  )}&name=${props.current.properties.theatre}`}
                  target="_blank"
                  rel="noreferrer"
                ...
                <a
                  href={`http://api.map.baidu.com/marker?location=${props.current.coordinates
                    .slice()
                    .reverse()
                    .join(",")}&title=${
                ...
                <a
                  href={`https://www.google.com/maps/search/?api=1&query=${props.current.coordinates
                    .slice()
                    .reverse()
                    .join(",")}`}
                  target="_blank"
                ...
                <a
                  href={`https://maps.apple.com/?q=${props.current.coordinates
                    .slice()
                    .reverse()
